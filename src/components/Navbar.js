  ...
  return (
    <div className="fixed top-1 left-1/2 -translate-x-1/2 z-50 w-full max-w-7xl px-3 sm:px-6" ref={mobileMenuRef}>
      <nav className="bg-[var(--nav-bg)] backdrop-blur-xl flex items-center justify-between py-2 px-2 sm:px-3 md:px-4 rounded-2xl border border-[var(--border-color)] shadow-lg">
        ...
        {/* ── DESKTOP menu (lg+) ── */}
        <div className="hidden lg:flex items-center gap-1 px-1 border border-[var(--border-color)] rounded-full py-1 shadow-xl bg-white/5 mx-4">
          {menuItems.map((item) => (
            <a
              key={item.id}
              href={item.href}
          ...
          <div className="relative group">
            <button className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-gray-400 hover:text-[var(--foreground)] hover:bg-white/5 transition-all">
        ...
        <div className="flex items-center gap-2 min-w-[60px] sm:min-w-[100px] justify-end relative z-[70]">
          {/* Theme toggle — only rendered on md+ (768px and above) */}
          {isTabletOrAbove && (
            <button
              onClick={toggleTheme}
              className="flex p-2.5 sm:p-3 rounded-full bg-white/5 border border-[var(--border-color)] text-gray-400 hover:text-[var(--foreground)] hover:bg-white/10 transition-all shadow-inner cursor-pointer"
          ...
          <button
            ...
            className="md:hidden p-2.5 rounded-full bg-white/5 border border-[var(--border-color)] text-gray-400 hover:text-[var(--foreground)] hover:bg-white/10 transition-all"
      ...
      <div
        className={`md:hidden mt-2 overflow-hidden transition-all duration-300 ease-in-out ${mobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
          }`}
      >
        <div className="bg-[var(--nav-bg)] backdrop-blur-xl border border-[var(--border-color)] rounded-2xl shadow-2xl p-3 space-y-1">
          {menuItems.map((item) => (
            <a
              key={item.id}
              href={item.href}
          ...
          {extraItems.map((item) => (
            <a
              ...
              href={item.href}
              onClick={() => setMobileMenuOpen(false)}
              className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-gray-400 hover:text-[var(--foreground)] hover:bg-white/5 transition"
            >
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-gray-400 hover:text-[var(--foreground)] hover:bg-white/5 transition"
          >
